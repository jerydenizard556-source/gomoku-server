...
if DATABASE_URL:
    try:
        ...
        c=db.cursor(); c.execute('''CREATE TABLE IF NOT EXISTS players (id SERIAL PRIMARY KEY, username VARCHAR(50) UNIQUE NOT NULL, email VARCHAR(255) UNIQUE NOT NULL, password_hash TEXT NOT NULL, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)'''); c.close(); print('PostgreSQL database connected.')
...
async def register(ws,u,e,p,pc):
    ...
    try:
        ...
        await send(ws,{'type':'register_result','success':True,'message':'Kont lan kreye avèk siksè.','player_id':pid,'username':u,'email':e,'account_token':t})
...
async def login(ws,l,p):
    ...
    try:
        c=db.cursor(); c.execute('SELECT id,username,email,password_hash FROM players WHERE LOWER(username)=LOWER(%s) OR LOWER(email)=LOWER(%s) LIMIT 1',(l,l)); row=c.fetchone(); c.close()
        ...
        t=secrets.token_urlsafe(32); set_account(ws,row[0],row[1],row[2],t); await send(ws,{'type':'login_result','success':True,'message':'Login reyisi.','player_id':row[0],'username':row[1],'email':row[2],'account_token':t})
...
def room_base(): return {'board':board(),'players':{},'spectators':set(),'turn':1,'starter':1,'black_time':START_TIME,'red_time':START_TIME,'last_timer_update':time.monotonic(),'game_over':False,'winner':0,'draw':False,'abandonment':False,'black_score':0,'red_score':0,'game_number':1,'last_move':None,'last_move_was_win':False,'winning_line':None,'rematch_request':None}
def new_room():
    c=code(); rooms[c]=room_base(); return c
def session(): return secrets.token_urlsafe(32)
def pstate(r,n):
    p=r['players'].get(n,{})
    return {'connected':p.get('connected',False),'username':p.get('username',f'Joueur {n}'),'name':p.get('username',f'Joueur {n}')}
def find(r,ws):
...
async def create(ws):
    c=new_room();r=rooms[c];t=session();r['players'][1]={'socket':ws,'connected':True,'session_token':t,'username':username(ws,'Joueur 1')};await send(ws,{'type':'created','room':c,'player':1,'session_token':t,'username':r['players'][1]['username']});await broadcast(r)
async def join(ws,c):
    ...
    t=session();r['players'][2]={'socket':ws,'connected':True,'session_token':t,'username':username(ws,'Joueur 2')};await send(ws,{'type':'joined','room':c,'player':2,'session_token':t,'username':r['players'][2]['username']});await broadcast(r)
async def find_match(ws):
    remove_spec(ws);queue[:]=[x for x in queue if x is not ws]
    if queue:
        a=queue.pop(0);c=code();r=room_base();rooms[c]=r
        n1=username(a,'Joueur 1');n2=username(ws,'Joueur 2');t1=session();t2=session();r['players']={1:{'socket':a,'connected':True,'session_token':t1,'username':n1},2:{'socket':ws,'connected':True,'session_token':t2,'username':n2}}
...
async def rematch_resp(ws,c,accepted):
    ...
    r['rematch_request']=None;r['last_timer_update']=time.monotonic();await broadcast(r)
async def reset(ws,c):
    c=str(c or '').upper().strip();
    if c not in rooms:return
    r=rooms[c];n=find(r,ws)
    if n is None or not r['game_over']:return
    r['starter']=2 if r['starter']==1 else 1;r['turn']=r['starter'];r['game_number']+=1;r['board']=board();r['black_time']=START_TIME;r['red_time']=START_TIME;r['game_over']=False;r['winner']=0;r['draw']=False;r['abandonment']=False;r['last_move']=None;r['last_move_was_win']=False;r['winning_line']=None;r['rematch_request']=None;await broadcast(r)
